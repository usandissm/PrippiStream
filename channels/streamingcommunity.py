...
def refresh_host_on_startup(timeout=3):
    ...
    global host
    candidates = []
    # The remote registry may have been atomically refreshed immediately
    # before Home starts. Read it first instead of waiting for a stale module
    # global to fail a request.
    try:
        registry_host = config.get_channel_url(name='streamingcommunity')
    except Exception:
        registry_host = ''
    for value in (registry_host, host, _WORKING_FALLBACK):
        value = (value or '').rstrip('/')
        if value and value not in candidates:
            candidates.append(value)
    deadline = time.monotonic() + max(1, float(timeout or 3))
    for candidate in candidates:
        try:
            ...
            response = httptools.downloadpage(candidate, follow_redirects=True,
                                              verify=False, timeout=max(1, remaining))
