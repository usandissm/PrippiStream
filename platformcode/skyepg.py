...
# Independent guide used for channels missing from Sky's DTH schedule and as a
# fallback when that API temporarily returns an incomplete event window.
_SUPERGUIDE_URLS = (
...
def _parse_superguide(html):
    """Return normalized channel -> current/next from SuperGuidaTV SSR HTML."""
    result = {}
...
def _superguide_for(key, entries):
    ...
    # Providers often use compact internal ids (rai1, mediaset20, skyuno...).
    for candidate in candidates:
        if candidate and candidate in entries:
            return entries[candidate]
    # Conservative fuzzy match: only accept containment for sufficiently long
    # names and choose the closest one, avoiding Rai/Sky numbered collisions.
...
def _prefetch(keys):
    ...
    with _epg_lock:
        for k, (cid, off) in res.items():
            ...
            n += 1
        # Fill every channel not covered by the official DTH guide.  Keeping
        # this inside the same refresh gives APK TV/mobile one coherent snapshot.
        for k in keys:
            nk = _norm(k)
...
def prefetch(keys):
    """Coalesce concurrent startup/page refreshes and refresh at most every 3m."""
