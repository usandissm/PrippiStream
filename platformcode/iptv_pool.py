...
def _bundled_seed_path(filename):
    """Private seed embedded by the local release builder.

    The public repository never tracks these JSON files. The packaged add-on
    receives them below ``resources/data`` so a fresh Kodi installation has
    the same IPTV catalog and pool as the Android app.
    """
    try:
        candidate = os.path.join(config.get_runtime_path(), 'resources', 'data', filename)
...
def _path():
    try:
        ...
        if os.path.isfile(data_path):
            return data_path
        runtime_path = next((candidate for candidate in _runtime_seed_paths('iptv_catalog.json')
                             if os.path.isfile(candidate)), '')
        return runtime_path or _bundled_seed_path('iptv_catalog.json') or None
