...
- [x] **FASE 6** — `reuselanguageinvoker` in addon.xml (dal 2° play si salta la catena import, ~1-3s su ARM); guardia `show_once` in launcher.run (niente write settings + broadcast a ogni click); pausa enrich bg durante avvio play e riproduzione (`_bg_ui_pause` clearato nei dispatch di `_launch`/`_play_4k_stream`, ri-settato in finally dal wrapper `_wait_and_restore`; wait nei 2 loop enrich); build 1.9.909 — ⏳ da testare (checklist FASE 6: 5+ play consecutivi cross-canale, resume/next-ep/lingua, focus restore, cambio setting live, uscita Kodi durante playback senza freeze)
